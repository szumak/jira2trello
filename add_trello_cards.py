...
def move_trello_card(card_id, list_id):
    ...
    response = requests.request("PUT", trello_url, params=query)
    #print(response.text)
...
def get_jira_issues():
    issues = jira.search_issues(
        'status in (Open, "In Progress", Reopened, "Waiting for support", "Waiting for customer", "Awaiting approval", Approved, "To Do", "AUTO - START", "On Hold") AND assignee in (currentUser())'
    )
    return issues


def get_trello_cards():
    trello_url = config['trello']['url'] + "boards/{}/cards".format(
        config['trello']['board_id'])
    query = {
        'key': config['trello']['key'],
        'token': config['trello']['token']
    }
    response = requests.request("GET", trello_url, params=query)
...
for issue in issues_list:
    ...
    if res == 0:
        print("adding card for {} - {}".format(issue.key,
                                               issue.fields.summary))
        add_trello_card(issue.key, '{}: {}'.format(issue.key,
                                                   issue.fields.summary))
